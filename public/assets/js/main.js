...
if (typeof $ !== 'undefined') {
    $(function () {
        ...
        if (searchInput.length) {
            ...
            var filterConfig = function (data) {
                return function findMatches(q, cb) {
                    ...
                    data.forEach(function (i) {
                        ...
                        if (searchStr.startsWith(queryStr)) {
                            matches.push(i);
                        } else if (searchStr.includes(queryStr)) {
                            matches.push(i);
                        }
                    });
            ...
            // Configuration des datasets pour typeahead
            var datasets = [];

            // Ajouter les formations si présentes
            if (searchData.courses && searchData.courses.length > 0) {
                datasets.push({
                    name: 'courses',
                    display: 'name',
                    limit: 4,
                    source: filterConfig(searchData.courses),
                    templates: {
                        header: '<h6 class="suggestions-header text-primary mb-0 mx-3 mt-3 pb-2"><i class="bx bx-book-open me-2"></i>Formations</h6>',
                        suggestion: function ({ name, icon, subtitle, url, id }) {
                            return (
                                '<a href="' + (typeof url === 'function' ? url(id) : url) + '">' +
                                '<div class="d-flex align-items-center">' +
                                // '<i class="bx ' + icon + ' me-3"></i>' +
                                '<div class="user-info">' +
            ...
            if (searchData.categories && searchData.categories.length > 0) {
                datasets.push({
                    ...
                    templates: {
                        header: '<h6 class="suggestions-header text-primary mb-0 mx-3 mt-3 pb-2"><i class="bx bx-category me-2"></i>Catégories</h6>',
                        suggestion: function ({ name, icon, subtitle, url, id }) {
                            return (
                                '<a href="' + (typeof url === 'function' ? url(id) : url) + '">' +
                                '<div class="d-flex align-items-center">' +
            ...
            // Initialiser typeahead seulement s'il y a des datasets
