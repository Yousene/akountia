...
@section('content')
    <div class="container-fluid flex-grow-1 container-p-y">
        ...
        <div class="card mb-4">
            ...
            <form class="card-body" method="POST" action="{{ route('review.update', ['review' => $record->id]) }}"
                enctype="multipart/form-data">
                @csrf
                @method('Put')
                <div class="row g-3">

                    @can('review.validation.update')
                        <div class="col-md-12">
                            ...
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="validation" name="validation"
                                    {{ old('validation', $record->validation) ? 'checked' : '' }}>
                            ...
                            <div class="d-flex gap-3 align-items-center">
                                ...
                                @if ($record->picture)
                                    <div class="d-flex align-items-center gap-2">
                                        <a href="{{ $record->picture }}" target="_blank">
                                            <img src="{{ $record->picture }}" class="rounded-circle"
                                                style="width: 60px; height: 60px; object-fit: cover;">
                                        </a>
                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input" name="remove_picture"
                                                id="remove_picture">
                                            <label class="form-check-label" for="remove_picture">Supprimer l'image</label>
                                        </div>
                                    </div>
                                @endif
                            </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="mt-2">
                                <div class="input-group" style="max-width: 150px;">
                                    <input type="number" name="rating" class="form-control" id="rating-input"
                    ...
                    @endcan
                    @can('review.genre.update')
                        <div class="col-md-1">
                            <label class="form-label" for="genre">Genre</label>
                            <select name="genre" id="genre" class="form-select">
                                <option value="">Sélectionner un genre</option>
                                <option value="Homme" {{ old('genre', $record->genre) == 'Homme' ? 'selected' : '' }}>Homme
                                </option>
                                <option value="Femme" {{ old('genre', $record->genre) == 'Femme' ? 'selected' : '' }}>Femme
                                </option>
                            </select>
                            @error('genre')
                                <span class="helper-text text-danger">{{ $message }}</span>
                    ...
                    @endcan
                    @can('review.company.update')
                        <div class="col-md-2">
                            <label class="form-label" for="company">Entreprise</label>
                            <input type="text" name="company" id="company" class="form-control"
                                placeholder="Entreprise" value="{{ old('company', $record->company) }}" />
                            @error('company')
                                <span class="helper-text text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                    @endcan
                    @can('review.company_url.update')
                        <div class="col-md-2">
                            ...
                            <input type="url" name="company_url" id="company_url" class="form-control"
                                placeholder="https://example.com" value="{{ old('company_url', $record->company_url) }}" />
                            @error('company_url')
                                <span class="helper-text text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                    @endcan
                    @can('review.position.update')
                        <div class="col-md-4">
                            <label class="form-label" for="position">Poste occupé</label>
                            <input type="text" name="position" id="position" class="form-control"
                                placeholder="Poste occupé" value="{{ old('position', $record->position) }}" />
                            @error('position')
                                <span class="helper-text text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="course_id" id="course_id" class="select2 form-control" data-allow-clear="true">
                                @foreach ($coursesRecords as $row)
                                    <option class="option"
                                        {{ $row->id == old('course_id', $record->course_id) ? 'selected' : '' }}
                    ...
                    @endcan
                    @can('review.comment.update')
                        <div class="col-md-6">
                            <label class="form-label" for="comment">Commentaire</label>
                            <input type="text" name="comment" id="comment" class="form-control"
                                placeholder="Commentaire" value="{{ old('comment', $record->comment) }}" />
                            @error('comment')
                                <span class="helper-text text-danger">{{ $message }}</span>
                            @enderror
                        </div>
