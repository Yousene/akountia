...
@section('content')
    <div class="container-fluid flex-grow-1 container-p-y">
        ...
        <div class="card mb-4">
            ...
            <form class="card-body" method="POST" action="{{ route('contact.update', ['contact' => $record->id]) }}">
                @csrf
                @method('Put')
                <div class="row g-3">
                    @can('contact.name.update')
                        <div class="col-md-6">
                    ...
                    @can('contact.email.update')
                        ...
                        @endcan @can('contact.subject.update')
                        <div class="col-md-12">
                        ...
                        @endcan @can('contact.message.update')
                        <div class="col-md-12">
                            <label class="form-label" for="message">Message</label>
                            <textarea name="message" id="message" class="form-control" placeholder="Message" rows="5">{{ old('message', $record->message) }}</textarea>
                            @error('message')
                                <span class="helper-text text-danger">{{ $message }}</span>
                            @enderror
                        </div>
